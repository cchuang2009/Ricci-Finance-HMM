...
def _edge_label_text(data: dict, metric: str) -> str:
    """Format the value shown at an edge midpoint."""
    correlation = float(data.get("correlation", np.nan))
    distance = float(data.get("distance", data.get("weight", np.nan)))
    curvature = float(data.get("ricciCurvature", np.nan))
    capital_flow = float(data.get("edge_capital_flow", np.nan))

    if metric == "correlation":
        return f"ρ={correlation:.2f}"
    if metric == "distance":
        return f"d={distance:.2f}"
    if metric == "capital_flow":
        return f"F={capital_flow:,.0f}"
    if metric == "all":
        return (
...
def _frame_traces(
    ...
    z_mode: str,
    edge_label_metric: str = "ricci",
    edge_label_top_n: int | None = None,
    ...
    traces = []
    flows = [
        float(data.get("edge_capital_flow", 0.0))
        for *_, data in G.edges(data=True)
    ]
    flow_scale = (
        np.nanpercentile(flows, 90)
        if flows and max(flows) > 0
        else 1.0
    )

    # One line trace per possible edge. Keeping this trace count fixed is
    # necessary for reliable Plotly animation updates.
    for u, v in all_edges:
        if G.has_edge(u, v):
            data = G[u][v]
            x0, y0, _ = positions.get(u, (0, 0, 0))
            x1, y1, _ = positions.get(v, (0, 0, 0))
            z0 = _node_z(G, u, z_mode)
            z1 = _node_z(G, v, z_mode)
            curvature = float(data.get("ricciCurvature", 0.0))
            flow = float(data.get("edge_capital_flow", 0.0))
            width = 1.0 + 6.0 * np.sqrt(
                max(flow, 0.0) / max(flow_scale, 1e-12)
            )
            traces.append(
                go.Scatter3d(
                    x=[x0, x1],
                    ...
                    line={
                        "width": width,
                        "color": ricci_to_hex(curvature),
                    },
                    hovertext=(
                        f"{u}–{v}<br>"
                        f"corr={data.get('correlation', np.nan):.3f}<br>"
                        f"distance={data.get('distance', np.nan):.3f}<br>"
                        f"Ricci={curvature:.3f}<br>"
                        f"capital flow={flow:,.0f}"
                    ),
                    hoverinfo="text",
                    showlegend=False,
                )
    ...
    # A single midpoint-text trace displays edge values. Unlike annotations,
    # Scatter3d text participates correctly in Plotly animation frames.
    label_candidates = []
    for u, v, data in G.edges(data=True):
        score = float(data.get("edge_capital_flow", 0.0))
        label_candidates.append((score, str(u), str(v), data))
    label_candidates.sort(reverse=True, key=lambda item: item[0])
    if edge_label_top_n is not None and edge_label_top_n > 0:
        label_candidates = label_candidates[: int(edge_label_top_n)]

    edge_x, edge_y, edge_z, edge_text, edge_hover = [], [], [], [], []
    if show_edge_labels:
        for _, u, v, data in label_candidates:
            x0, y0, _ = positions.get(u, (0, 0, 0))
            x1, y1, _ = positions.get(v, (0, 0, 0))
            z0 = _node_z(G, u, z_mode)
            z1 = _node_z(G, v, z_mode)
            edge_x.append((x0 + x1) / 2.0)
            edge_y.append((y0 + y1) / 2.0)
            edge_z.append((z0 + z1) / 2.0)
            edge_text.append(_edge_label_text(data, edge_label_metric))
            edge_hover.append(
                f"{u}–{v}<br>"
                f"correlation={float(data.get('correlation', np.nan)):.3f}<br>"
                f"distance={float(data.get('distance', np.nan)):.3f}<br>"
                f"Ricci κ={float(data.get('ricciCurvature', np.nan)):.3f}<br>"
                f"capital flow={float(data.get('edge_capital_flow', np.nan)):,.0f}"
            )
    ...
    masses = np.asarray(
        [float(G.nodes[node].get("capital_mass", 0.0)) for node in G.nodes()]
    )
    mass_scale = (
        np.nanpercentile(masses[masses > 0], 90)
        if np.any(masses > 0)
        else 1.0
    )
    xs, ys, zs, labels, colors, sizes, hover = [], [], [], [], [], [], []
    for node in sorted(G.nodes()):
        x, y, _ = positions.get(node, (0, 0, 0))
        z = _node_z(G, node, z_mode)
        cluster_id = fd.node_cluster.get(node, 0)
        status = G.nodes[node].get("status", "active")
        mass = float(G.nodes[node].get("capital_mass", 0.0))
        xs.append(x)
        ys.append(y)
        zs.append(z)
        labels.append(str(node) if show_node_labels else "")
        colors.append(
            "#D0D0D0"
            if status != "active"
            else COMPONENT_COLORS[cluster_id % len(COMPONENT_COLORS)]
        )
        sizes.append(
            8 + 22 * np.sqrt(max(mass, 0.0) / max(mass_scale, 1e-12))
        )
        hover.append(
            f"Ticker: {node}<br>"
            f"cluster={cluster_id}<br>"
            f"status={status}<br>"
            f"capital mass={mass:,.0f}<br>"
            f"z={z:.3f}"
        )
...
def build_3d_ricci_capital_animation(
    ...
    show_node_labels: bool = True,
    show_edge_labels: bool = True,
    title: str = "Ricci Finance v13: dynamic 3D Ricci-capital manifold",
