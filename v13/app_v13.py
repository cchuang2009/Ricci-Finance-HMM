...
st.set_page_config(page_title="Ricci Finance v13", layout="wide")
st.title("Ricci Finance v13 — Multi-Engine Regime Manifold")
st.caption(
    "Ricci-capital rolling graphs with hmmlearn Viterbi/posterior decoding, "
    "duration-aware HSMM, optional pomegranate posterior decoding, and model consensus."
)

with st.sidebar:
    source = st.radio("Data source", ["Synthetic", "yfinance"], index=0)
...
try:
    if source == "Synthetic":
        close, volume, dollar_volume = make_demo_market_data(tickers, n_days=340)
    else:
...
with main_tabs[0]:
    st.plotly_chart(
        build_3d_ricci_capital_animation(
            ...
            stories=stories,
            title=f"Ricci Finance v13 · {engine_choice}",
        ),
        width="stretch",
    )
    st.dataframe(
