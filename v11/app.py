...
from helper import (
    ...
    ricci_visual_caption,
)


st.set_page_config(page_title="Ricci Finance v11", layout="wide")

st.title("Ricci Finance v11.4: Dynamic 3D Ricci-Capital Manifold")
st.caption(
...
with st.expander("v11 visual encoding: what the edge colors mean", expanded=True):
    st.markdown(
        f"""
        **Edge color is not arbitrary.** {ricci_visual_caption()}

        **Visual channels used in v11**

        | Visual property | Meaning |
        |---|---|
        | Edge color | Ollivier-Ricci curvature κ; red = negative, blue = positive |
        | Edge thickness | Estimated dollar-volume capital transport |
        | Edge label `w` | Financial distance / current Ricci-flow weight |
        | Node size | Dollar-volume capital mass |
        | Node color | Connected component / cluster |
        | Gray node | Waiting for enough valid data, e.g. pre-IPO period |
        | 3D z-axis | Selected z mode: Ricci stress, capital mass, or spring layout |
        """
    )
...
if enable_3d:
    ...
    st.caption("x/y = stable topology, z = Ricci stress or selected mode, node size = dollar-volume capital mass, edge width = capital transport. " + ricci_visual_caption())
...
st.caption("Play the rolling-window market geometry. " + ricci_visual_caption() + " Edge labels show current financial distance / weight.")
