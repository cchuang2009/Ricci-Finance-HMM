...
Rolling Ricci financial network v8 with Plotly animation, IPO nodes, HMM hidden market regimes, and Ricci flow diagnostics.
...
10. Ricci flow diagnostics on any selected animation frame.
...
from helper import run_ricci_flow, plot_ricci_flow_history, compare_before_after_flow, visualize_network
...
def build_plotly_figure(
    ...
    fig.update_layout(
        title="Rolling Ricci Financial Network v8 - Plotly Animation + HMM + Ricci Flow",
        width=None,
        height=780,
        showlegend=False,
        hovermode="closest",
...
def main() -> None:
    st.set_page_config(page_title="Rolling Ricci Financial Network v8 HMM + Flow", layout="wide")
    st.title("Rolling Ricci Financial Network v8 + HMM + Ricci Flow")
    st.caption("v8 keeps the v7 Plotly animation design, then adds Ricci flow as a geometric stress-test layer for the selected frame.")
    ...
    with st.sidebar:
        ...
        st.header("Ricci flow v8")
        enable_flow = st.checkbox("Enable Ricci flow diagnostics", value=True)
    ...
    if enable_flow:
        st.subheader("Ricci flow v8: deform selected frame")
        st.markdown(
            "Ricci flow updates financial distances by curvature: "
            "`new_distance = old_distance × (1 - step_size × Ricci)`. "
            "Positive-curvature cluster links contract; negative-curvature bridge/stress links stretch."
        )
        with st.spinner("Running Ricci flow on selected frame..."):
        ...
        with right:
            ...
            st.plotly_chart(
                visualize_network(flowed_G, positions=positions, title="After Ricci flow", node_cluster=compute_components(flowed_G)),
                width="stretch",
            )

        with st.expander("Before/after Ricci-flow edge comparison", expanded=True):
            flow_compare = compare_before_after_flow(fd.G, flowed_G)
            st.dataframe(flow_compare, width="stretch")
            if not flow_compare.empty:
                st.download_button(
                    "Download Ricci-flow comparison CSV",
                    data=flow_compare.to_csv(index=False).encode("utf-8"),
                    file_name="ricci_flow_comparison_v8.csv",
                    mime="text/csv",
                )
