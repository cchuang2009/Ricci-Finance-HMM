...
RICCI_COLOR_MIN = -0.6
RICCI_COLOR_MAX = 0.6
RICCI_PLOTLY_COLORSCALE = "RdBu"  # low/negative = red, high/positive = blue


def ricci_to_hex(kappa: float, vmin: float = RICCI_COLOR_MIN, vmax: float = RICCI_COLOR_MAX) -> str:
    """Map Ricci curvature to a stable lecture color scale.

    Negative curvature is red: fragile bridge / stress-transmission edge.
    Near-zero curvature is white: neutral geometry.
    Positive curvature is blue: coherent local capital basin.
    The fixed range is intentionally shared by all 2D/3D/static/animated figures.
    """
...
def ricci_colorbar_trace_2d(title: str = "Ricci κ<br>red− / blue+") -> go.Scatter:
    """Invisible 2D trace that adds a fixed Ricci-curvature colorbar."""
    return go.Scatter(
        x=[None, None],
        y=[None, None],
        mode="markers",
        marker={
            ...
            "cmin": RICCI_COLOR_MIN,
            "cmax": RICCI_COLOR_MAX,
            "showscale": True,
            "colorbar": {"title": title, "thickness": 16, "len": 0.62},
        },
        hoverinfo="skip",
        showlegend=False,
    )
...
def ricci_colorbar_trace_3d(title: str = "Ricci κ<br>red− / blue+") -> go.Scatter3d:
    """Invisible 3D trace that adds a fixed Ricci-curvature colorbar."""
    return go.Scatter3d(
        x=[None, None],
        y=[None, None],
        z=[None, None],
...
def ricci_visual_caption() -> str:
    return (
        "Edge color uses a fixed Ricci-curvature scale: red = negative curvature "
        "(fragile/stress bridge), white = near neutral, blue = positive curvature "
        "(coherent local capital basin). The fixed range is κ ∈ [-0.6, +0.6]."
    )
...
def visualize_network(
    ...
    fig = go.Figure(data=edge_traces + [edge_label_trace, node_trace, ricci_colorbar_trace_2d()])
    fig.update_layout(
        title=title,
        height=650,
        hovermode="closest",
...
def build_plotly_animation(
    ...
    node_size_base: int = 24,
    edge_width_scale: float = 5.0,
    title: str = "Rolling Ricci Finance v11",
    show_edge_weight_labels: bool = True,
    edge_label_top_n: int = 30,
) -> go.Figure:
    ...
    fig.add_trace(ricci_colorbar_trace_2d())
    fig.frames = [
        go.Frame(
            name=str(i),
            data=_frame_traces(
...
def build_3d_ricci_capital_animation(
    ...
    fig.add_trace(ricci_colorbar_trace_3d())
    fig.frames = [
        go.Frame(
            name=str(i),
...
def visualize_network_3d(
    ...
    fig = go.Figure(data=_frame_traces_3d(fd, positions_3d, all_nodes, all_edges, z_mode=z_mode) + [ricci_colorbar_trace_3d()])
    fig.update_layout(
        title=title,
        height=800,
        showlegend=False,
