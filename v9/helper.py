...
def make_demo_prices(
    tickers: Sequence[str] = DEFAULT_TICKERS[:12],
    n_days: int = 260,
    seed: int = 7,
    ipo_aware: bool = True,
) -> pd.DataFrame:
    """Synthetic lecture data with IPO-aware late-start tickers.

    In v8, late-start tickers such as QNT only appeared after they had
    enough valid observations inside the rolling window.  v9 keeps that
    behavior by inserting NaN prices before a synthetic IPO/start date.

    The layout can still reserve a stable coordinate for every requested
    ticker, but a node is drawn in a frame only when that ticker has at
    least `min_node_obs` valid returns in that frame.
    """
    ...
    # Temporary market shock around 60%-75% of sample.
    shock_start, shock_end = int(0.60 * n_days), int(0.75 * n_days)
    factors[shock_start:shock_end, 2] += rng.normal(-0.006, 0.030, size=shock_end - shock_start)

    loadings = rng.uniform(-0.2, 1.1, size=(n, 3))
    for i in range(n):
        loadings[i, i % 3] += 0.9

    noise = rng.normal(0, 0.012, size=(n_days, n))
    ...
    if ipo_aware:
        # Synthetic IPO / late-start calendar.  Add symbols here when the
        # class wants to demonstrate new tickers entering the market manifold.
        late_start_fraction = {
            "QNT": 0.70,
            "BNT": 0.70,
            "SPCX": 0.76,
        ...
        for symbol, frac in late_start_fraction.items():
            if symbol in df.columns:
                start_idx = min(max(1, int(frac * n_days)), n_days - 2)
                df.loc[df.index[:start_idx], symbol] = np.nan
                # Rebase visible post-IPO price path so it starts naturally near 100.
                first_valid = df.index[start_idx]
                base = df.loc[first_valid, symbol]
